...
{
	_my_eputs(info->fname);
	_my_eputs(": ");
	my_print_d(info->line_count, STDERR_FILENO);
	_my_eputs(": ");
	_my_eputs(info->argv[0]);
	_my_eputs(": ");
	_my_eputs(estr);
}
...
{
	int (*__my_putchar)(char) = _my_putchar;
	int i, count = 0;
	unsigned int _abs_, current;

	if (fd == STDERR_FILENO)
		__my_putchar = _my_eputchar;
	if (input < 0)
	{
		_abs_ = -input;
		__my_putchar('-');
	...
	for (i = 1000000000; i > 1; i /= 10)
	{
		if (_abs_ / i)
		{
			__my_putchar('0' + current / i);
			count++;
		}
		current %= i;
	}
	__my_putchar('0' + current);
	count++;
...
char *my_convert_number(long int num, int base, int flags)
{
	...
	array = flags & MY_CONVERT_LOWERCASE ? "0123456789abcdef"
		: "0123456789ABCDEF";
	ptr = &buffer[49];
	*ptr = '\0';
