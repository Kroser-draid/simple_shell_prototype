...
char *get_history_file(my_info_t *info)
{
	...
	if (!dir)
		return (NULL);
	buf = malloc(sizeof(char) * (_my_strlen(dir) + _my_strlen(MY_HIST_FILE) + 2));
	if (!buf)
		return (NULL);
	buf[0] = 0;
	_my_strcpy(buf, dir);
	_my_strcat(buf, "/");
	_my_strcat(buf, MY_HIST_FILE);
	return (buf);
}
...
/**
*renumber_history - remumbers the history linked list after changes
*@info: structure containing potential arguments. used to maintain
*
